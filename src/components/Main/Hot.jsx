...
const Hot = ({data, isFetching}) => {
    ...
    const shuffleArray = ()=>{
        setStart(oldStart => (data.length - (oldStart + 7)) < 7 ? data.length - 7 : oldStart + 7)
    }
    const oldArray = ()=>{
        ...
        <>
            {isFetching ? <div style={{height: '340px', paddingTop: '150px'}}><Loader fix={false}/></div> :
                <div className="container">
                    <div className={style.best + ' row ' + style.hot}>
                                ...
                                {data6.map((value) => {
                                    return (
                                        <div className={style.item} key={value.id}>
                                            ...
                                            <Hyphenated>
                                                <NavLink to={BOOK_ROUTE + '/' + value.id}><p className='p5'>

                                                    {nameLengthSlice(value.name, 17)}
                                ...
                                {oneBook ?
                                    <>
                                        <div className={style.item}>
                                            ...
                                            <Hyphenated>
                                                <NavLink to={BOOK_ROUTE + '/' + oneBook.id}><p className='p4'>
